...
export function PIPE_HOVERED(pipeRef) {
...
export function UNSELECT_VIEW_NODE(this: any, selfOnly, stopPropagation, e) {
    if (stopPropagation) {
        e.stopPropagation()
    }
...
export function UNSELECT_STATE_NODE(this: any, e) {
    if (e.target === this) {
...
export function ADD_DEFAULT_TRANSFORMATION(pipeId) {
    ...
    if (stateInPipe.type === 'table') {
        ...
        const row = {
            table: pipe.value,
            columns: stateInPipe.columns.map(_ => {
                return {
                    ref: 'column',
                    id: uuid(),
                }
    ...
    setState(
        R.evolve({
            definitionList: {
                [state.currentDefinitionId]: {
                    [transformation]: R.assocPath([newId, 'value'], {
                        ref: 'pipe',
                        id: newPipeId,
                    }),
                    pipe: R.pipe(
...
export function CHANGE_TRANSFORMATION(pipeRef, oldTransformationRef, _, e) {
...
export function UPDATE_TABLE_DEFAULT_RECORD(tableId) {
    const currentDefinition = state.definitionList[state.currentDefinitionId]
    const table = currentDefinition.table[tableId]
    let defaultRow = {} as any
...
export function UPDATE_TABLE_ADD_COLUMN(tableId, type) {
    ...
    let addedPipes = {}
    let addedColumns = {}
    const updatedRows = R.mapObjIndexed(row => {
