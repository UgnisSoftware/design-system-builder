...
import { HOVER_MOBILE, EDIT_VIEW_NODE_TITLE, VIEW_NODE_SELECTED, VIEW_UNHOVERED, VIEW_HOVERED, VIEW_DRAGGED } from '../../../../events'
...
export default function UgnisComponent({ nodeRef, parentRef, depth }) {
    ...
    return (
        <div
            style={{
                opacity: state.draggedComponentView && state.draggedComponentView.id === nodeId ? 0.5 : 1,
            }}
        >
            <div
                style={{
                  ...
                  !(node.children.findIndex(ref => ref.id === state.draggedComponentView.id) === state.hoveredViewNode.position)
                    ? (() => {
                          ...
                          const children = node.children.map(ref => (
                              <UgnisComponent key={ref.id} nodeRef={ref} parentRef={nodeRef} depth={depth + 1} />
                          ))
                          return children.slice(0, newPosition).concat(<SpacerComponent />, children.slice(newPosition))
                      })()
                    : node.children.map(ref => <UgnisComponent key={ref.id} nodeRef={ref} parentRef={nodeRef} depth={depth + 1} />)
                : ''}
        </div>
    )
}
