...
import { cx, mergeWith, __DEV__ } from "~/utils"
...
export const ScaleFade = React.forwardRef<HTMLDivElement, ScaleFadeProps>((props, ref) => {
  const { unmountOnExit, in: isOpen, reverse = true, initialScale = 0.95, className, ...rest } = props
  ...
  return (
    <AnimatePresence custom={custom}>
      {show && <motion.div ref={ref} className={cx("chakra-offset-slide", className)} {...motionProps} {...rest} />}
    </AnimatePresence>
  )
})
