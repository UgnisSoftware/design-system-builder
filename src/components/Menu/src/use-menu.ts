...
import { useDescendant, useDescendants } from "~/descendant"
import {
...
import { createContext, EventKeyMap, getValidChildren, mergeRefs } from "~/react-utils"
import {
  addItem,
  callAllHandlers,
  dataAttr,
...
export function useMenu(props: UseMenuProps) {
  const { id, closeOnSelect = true, closeOnBlur = true, autoSelect = true, isLazy, placement = "bottom-start" } = props
  ...
  useOutsideClick({
    ref: menuRef,
    handler: (event) => {
      if (isOpen && closeOnBlur && !buttonRef.current?.contains(event.target as HTMLElement)) {
        onClose()
      }
    },
  })
...
export function useMenuButton(props: UseMenuButtonProps, externalRef: React.Ref<any> = null) {
  const menu = useMenuContext()

  const { isOpen, onClose, autoSelect, popper, openAndFocusFirstItem, openAndFocusLastItem, openAndFocusMenu } = menu
...
export function useMenuList(props: UseMenuListProps, ref: React.Ref<any> = null) {
  const menu = useMenuContext()
...
export function useMenuItem(props: UseMenuItemProps, externalRef: React.Ref<any> = null) {
  const {
  ...
  const { domContext, setFocusedIndex, focusedIndex, closeOnSelect, onClose, menuRef, isOpen } = menu
...
export function useMenuOption(props: UseMenuOptionProps, externalRef: React.Ref<any> = null) {
  const { onClick, isDisabled, isFocusable, type = "radio", isChecked, ...rest } = props
