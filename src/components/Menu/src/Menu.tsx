...
import { cx, runIfFn, __DEV__ } from "~/utils"
import type { MaybeRenderProp } from "~/react-utils"
...
export const Menu: React.FC<MenuProps> = (props) => {
  ...
  return (
    <MenuProvider value={context}>
      <StylesProvider value={styles}>{runIfFn(children, { isOpen, onClose, forceUpdate })}</StylesProvider>
    </MenuProvider>
  )
}
...
export const MenuButton = forwardRef<MenuButtonProps, "button">((props, ref) => {
  ...
  return (
    <Element {...buttonProps} className={cx("chakra-menu__menu-button", props.className)}>
      <chakra.span __css={{ pointerEvents: "none", flex: "1 1 auto" }}>{props.children}</chakra.span>
    </Element>
  )
})
...
const MotionDiv: CustomDomComponent<PropsOf<typeof chakra.div>> = (motion as any)(chakra.div)
...
export const MenuList = forwardRef<MenuListProps, "div">((props, ref) => {
  ...
  return (
    <chakra.div {...positionerProps} __css={{ zIndex: props.zIndex ?? styles.list?.zIndex }}>
      <MotionDiv
        {...listProps}
        /**
...
interface MenuItemOptions extends Pick<UseMenuItemProps, "isDisabled" | "isFocusable"> {
  /**
   * The icon to render before the menu item's text.
   * @type React.ReactElement
   */
  icon?: React.ReactElement
  /**
   * The spacing between the icon and menu item's text.
   * @type SystemProps["mr"]
   */
  iconSpacing?: SystemProps["mr"]
  /**
   * Right-aligned text text content, useful for displaying hotkeys.
   */
  command?: string
  /**
   * The spacing between the command and menu item's text.
   * @type SystemProps["ml"]
   */
...
export const MenuItem = forwardRef<MenuItemProps, "button">((props, ref) => {
  ...
  return (
    <StyledMenuItem {...menuItemProps} className={cx("chakra-menu__menuitem", menuItemProps.className)}>
      {icon && (
...
export const MenuItemOption = forwardRef<MenuItemOptionProps, "button">((props, ref) => {
  ...
  return (
    <StyledMenuItem {...optionProps} className={cx("chakra-menu__menuitem-option", rest.className)}>
      <MenuIcon fontSize="0.8em" marginEnd={iconSpacing} opacity={props.isChecked ? 1 : 0}>
        {icon || <CheckIcon />}
      </MenuIcon>
...
export const MenuOptionGroup: React.FC<MenuOptionGroupProps> = (props) => {
  ...
  return <MenuGroup title={title} className={cx("chakra-menu__option-group", className)} {...ownProps} />
}
...
export const MenuCommand = forwardRef<MenuCommandProps, "span">((props, ref) => {
  const styles = useStyles()
  return <chakra.span ref={ref} {...props} __css={styles.command} className="chakra-menu__command" />
})
