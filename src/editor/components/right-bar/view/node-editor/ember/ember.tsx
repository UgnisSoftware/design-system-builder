...
export default function emberEditor(ref, type?) {
            ...
            >
                {type.values.map((value, index) => (
                    <div
                        ...
                        onClick={_ => CHANGE_STATIC_VALUE(ref, 'value', 'text', { target: { value } })}
                    >
                        {value}
                    </div>
                ))}
            ...
            <div style={{ display: 'flex', alignItems: 'baseline' }} onClick={e => SELECT_PIPE(ref.id, e)}>
                ...
                >
                    ...
                    <input
                        ...
                        onMouseMove={_ => PIPE_HOVERED(ref)}
    ...
    if (!isNaN(parseFloat(pipe.value)) && isFinite(Number(pipe.value))) {
        return (
            <div style={{ display: 'flex', alignItems: 'baseline' }} onClick={e => SELECT_PIPE(ref.id, e)}>
                <span
                    style={{
                ...
                >
                        ...
                        style={{
                            ...
                            borderBottom:
                                ...
                                (pipe.transformations.length > 0
                                    ? state.selectedStateNode.id === pipe.value.id
                                        ? '#53d486'
                                        : '#eab65c'
                                    : '#ccc'),
                        }}
                        onClick={_ => STATE_NODE_SELECTED(pipe.value)}
                    >
                        {displState.title}
                    </span>
                    {state.selectedPipeId === ref.id ? (
                        <span
                            ...
                            onClick={_ => ADD_DEFAULT_TRANSFORMATION(state.selectedPipeId)}
                        >
                            <AddCircleIcon />
                        </span>
                    ) : (
    ...
    if (pipe.value.ref === 'eventData') {
        return (
            <div>
                <div onClick={e => SELECT_PIPE(ref.id, e)} onMouseMove={_ => PIPE_HOVERED(ref)} onMouseOut={PIPE_UNHOVERED}>
                    <div style={{ flex: '1' }}>
                        <div
                            style={{
                                cursor: 'pointer',
