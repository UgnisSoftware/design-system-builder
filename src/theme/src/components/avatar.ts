import { isDark, mode, randomColor } from "../../../theme-tools"
...
type Size = keyof typeof themeSizes

function isSize(value: any): value is Size {
  return (themeSizes as any)[value]
}

function getSize(size: unknown) {
  let themeSize
  if (isSize(size)) {
    themeSize = themeSizes[size]
  }
  return {
    container: {
      width: size,
