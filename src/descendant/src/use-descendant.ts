...
export function useDescendant<T extends HTMLElement, P>(props: UseDescendantProps<T, P>) {
  const { context, element, index: indexProp, disabled, focusable, ...rest } = props
  ...
  const index = indexProp ?? descendants.findIndex((descendant) => descendant.element === element)
...
export function useDescendants<T extends HTMLElement, P>() {
  ...
  const context = useMemo(() => ({ descendants, register, unregister }), [descendants, register, unregister])
